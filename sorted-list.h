 ...
 *
 * The comparator function will take pointers to two data items and will return -1 if the 	* 1st is smaller, 0 if the two are equal, and 1 if the 2nd is smaller.
 *
 * The user will also supply a destruct function will take a pointer to a single data item
 * and knows how to deallocate it. If the data item does not require deallocation, the
 * user's destruct function will do nothing.
 *
...
/*  EXAMPLE:
int anINtCompareFunc(void * firstArg, void * secondArg)
{
...
/*	EXAMPLE of main function
		TEST structs, char array/string, char, int, float, ptr?, functptr?
	main(){
		//create functPtr to comparator and destructor
		SortedListPtr = createSortedList(fnprttocomparator, fnptrtodestructor)

		int * newval = (int*)malloc(sizeof(int));
		*newval = 5;
		void * sortthing = (void*) newval;

		SLInsert(SortedListPtr, sorrthing);
	}
*/
...
//build the SortedList struct and hand back a pointer to it.
